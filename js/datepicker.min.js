...
/*@cc_on
                ...
                th.unselectable = "on";
                /*@end@*/
return th}function createThAndButton(tr,obj){for(var i=0,details;details=obj[i];i++){var th=createTH(details);tr.appendChild(th);var but=document.createElement("a");but.className=details.className;but.id=o.id+details.id;but.href="#"+but.id;but.innerHTML=details.text||o.nbsp;but.title=details.title||"";
/*@cc_on
                    /*@if(@_win32)
                    th.unselectable = but.unselectable = "on";
                    /*@end@*/
th.appendChild(but)}}this.div=document.createElement("div");this.div.id="fd-"+this.id;this.div.className="date-picker"+this.bespokeClass;this.div.style.visibility="hidden";this.div.style.display="none";if(this.describedBy&&document.getElementById(this.describedBy)){setARIAProperty(this.div,"describedby",this.describedBy)}if(this.labelledBy){setARIAProperty(this.div,"labelledby",this.labelledBy.id)}var tr,row,col,tableHead,tableBody,tableFoot;this.table=document.createElement("table");this.table.className="date-picker-table";this.table.onmouseover=this.onmouseover;this.table.onmouseout=this.onmouseout;this.table.onclick=this.onclick;if(this.staticPos){this.table.onmousedown=this.onmousedown}this.div.appendChild(this.table);var dragEnabledCN=!this.dragDisabled?" drag-enabled":"";if(!this.staticPos){this.div.style.visibility="hidden";this.div.className+=dragEnabledCN;var input=document.getElementById(this.id);document.getElementsByTagName("body")[0].appendChild(this.div);input.parentNode.appendChild(this.div);
/*@cc_on
                @if(@_jscript_version <= 5.7)
...
/*@cc_on
            ...
            @end
            @*/
this.opacityTo=0;this.fade()};this.onblur=function(e){};this.onfocus=function(e){o.noFocus=false;addClass(o.div,"date-picker-focus");if(o.statusBar){o.updateStatus(printFormattedDate(o.date,o.statusFormat,true))}o.addOnFocusEvents()};this.onmousewheel=function(e){e=e||document.parentWindow.event;var delta=0;if(e.wheelDelta){delta=e.wheelDelta/120;if(isOpera&&window.opera.version()<9.2){delta=-delta}}else{if(e.detail){delta=-e.detail/3}}var n=o.date.getDate(),d=new Date(o.date),inc=delta>0?1:-1;d.setDate(2);d.setMonth(d.getMonth()+inc*1);d.setDate(Math.min(n,daysInMonth(d.getMonth(),d.getFullYear())));if(o.outOfRange(d)){return stopEvent(e)}o.date=new Date(d);o.updateTable();if(o.statusBar){o.updateStatus(printFormattedDate(o.date,o.statusFormat,true))}return stopEvent(e)};this.onkeydown=function(e){o.stopTimer();if(!o.visible){return false}e=e||document.parentWindow.event;var kc=e.keyCode?e.keyCode:e.charCode;if(kc==13){var td=document.getElementById(o.id+"-date-picker-hover");if(td===document.activeElement){if(!td||td.className.search(/cd-([0-9]{8})/)==-1||td.className.search(/out-of-range|day-disabled/)!=-1){return stopEvent(e)}o.dateSet=new Date(o.date);o.callback("dateset",o.createCbArgObj());o.returnFormattedDate();var button=document.getElementById("fd-but-"+o.id);button.setAttribute("title",getTitleTranslation(5));var spans=button.getElementsByTagName("span");var label=spans[spans.length-1];label.innerHTML=getTitleTranslation(5);o.hide();return stopEvent(e)}}else{if(kc==27){if(!o.staticPos){var button=document.getElementById("fd-but-"+o.id);button.setAttribute("title",getTitleTranslation(5));var spans=button.getElementsByTagName("span");var label=spans[spans.length-1];label.innerHTML=getTitleTranslation(5);o.hide();var butt=document.getElementById("fd-but-"+o.id);if(butt){butt.focus()}return stopEvent(e)}return true}else{if(kc==32||kc==0){o.date=new Date();o.updateTable();return stopEvent(e)}else{if(kc==9){}}}}
/*@cc_on
            @if(@_win32)
